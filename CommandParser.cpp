...
void CommandParser::parseCommand(const std::string& msg, Client* client, ServerSocket& server, int epoll_fd, Bot& bot)
{
    ...
    else
    {
        ...
        {
            handlePrivmsg(ircMsg, client, server, bot);
        }
        ...
        {
            handleKick(ircMsg, client, server);
        }
        else if (ircMsg.command == "INVITE")
        {
            handleInvite(ircMsg, client, server);
        }
    ...
    else if (msg.params[1] == "+l")
    {
        ...
        if (!(ss >> limit))
        {
            server.sendMessage(client->getFd(),
                "399  Invalid parameter for +l mode\r\n");
            return;
        }
...
void CommandParser::handleInvite(const IRCMessage& msg, Client* client, ServerSocket& server)
{
    if (!client->getIsAuthenticated() || client->getNickname().empty())
    {
        server.sendMessage(client->getFd(), ":server 451 * :You have not registered\r\n");
    ...
    {
        server.sendMessage(client->getFd(), ":server 401 " + client->getNickname() + " " +
                         msg.params[1] + " :No such nick\r\n");
        return;
    }

    if (channel->isMember(client2->getFd()))
    {
        server.sendMessage(client->getFd(), ":server 443 " + client->getNickname() + " " +
                         msg.params[1] + " " + channelName + " :is already on channel\r\n");
        return;
    }
    client2->setIninvited(1);
    server.sendMessage(client2->getFd(),
        ":" + client->getNickname() + "!" + client->getUsername() + "@localhost" +
        " INVITE " +  msg.params[1] + " :" + channelName + "\r\n");
}

//Removes a user from a channel
void CommandParser::handleKick(const IRCMessage& msg, Client* client, ServerSocket& server)
{
    //KICk  <channle> <user> [<comment>]
    ...
    std::string reason = msg.trailing.empty() ? "Kicked" : msg.trailing;
    std::string msg_ = ":" + client->getNickname() + "!" + client->getUsername() +
                          "@localhost" + " KICK " + channelName + " " +
                           msg.params[1] + " :" + reason + "\r\n";
    Client* client2 = server.getClientManager().getClientByNickname(msg.params[1]);
    if (!client2)
    {
        server.sendMessage(client->getFd(), ":server 441 " + client->getNickname() + 
                        " " + msg.params[1] + " :They aren't on the channel\r\n");
        return;
    }
    channel->removeMember(client2->getFd());
    client2->removeChannel(channelName);
    broadcastToChannel(channelName, msg_, server);
...
void CommandParser::handleTopic(const IRCMessage& msg, Client* client, ServerSocket& server)
{
    ...
    {
        if (channel->isTopicRestricted() == 1)
        {
            if (!channel->isOperator(client->getFd()))
            {
                server.sendMessage(client->getFd(), ":server 482 " + client->getNickname() + 
